...
class WebScrapper(scrapy.Spider):
    ...
    def Product_details_on_first_page(self,asin,response):
        ...
        collection.insert_one({"ASIN": asin, "title": Product_title, "url": Product_link,
                               "Paperback": [{"price": Paperback, "time": datetime.utcnow()}],
                               "Kindle_Edition": [{"price": Kindle_Edition, "time": datetime.utcnow()}],
                               "Hardcover": [{"price": Hardcover, "time": datetime.utcnow()}]})
    ...
    def Product_details_on_next_page(self,asin_next,response):
        ...
        collection.insert_one({"ASIN": asin_next, "title": Product_title_next, "url": Product_link_next,
                               "Paperback": [{"price":Paperback_next,"time":datetime.utcnow()}],
                               "Kindle_Edition": [{"price":Kindle_Edition_next, "time": datetime.utcnow()}],
                               "Hardcover": [{"price":Hardcover_next, "time": datetime.utcnow()}]})

    def get_Details_of_Primary_Page(self,response):
        try:
            ASIN = response.xpath('.//li[@class = "s-result-item celwidget  "]/@data-asin').extract()
            for asin in ASIN:
                if asin in asins or collection.find({"_id":asin}).count() > 0:
                    pass
                else:
            ...
            for asin_next in ASIN_next:
                if asin_next in asins or collection.find({"_id":asin_next}).count() > 0:
                    pass
                else:
